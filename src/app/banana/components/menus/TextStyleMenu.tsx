...
const StyleDropdown = ({ 
  ...
  return createPortal(
    <div 
      ref={dropdownRef}
      className="bg-white shadow-lg rounded-md border border-gray-200 w-40 h-fit overflow-y-auto"
      style={{
        position: 'absolute',
        top: `${position.top}px`,
    ...
    >
      {styles.map(style => (
        <button
          ...
          className={`block w-full text-left px-4 py-2 hover:bg-gray-100 text-black ${currentStyle === style.id ? 'bg-gray-100' : ''}`}
        >
          <div 
          >
            {style.label}
...
const TextStyleMenu: React.FC<TextStyleMenuProps> = ({
  ...
  const toggleTextDecoration = (decoration: 'underline' | 'line-through' | 'none') => {
    if (decoration === 'underline' && applyFormatToSelection('underline')) {
      // The formatting was applied to the selection
      console.log("Applied underline formatting to selection");
    } else if (decoration === 'line-through' && applyFormatToSelection('strikeThrough')) {
      // The formatting was applied to the selection
  ...
  const setTextColor = (color: string) => {
    if (applyFormatToSelection('foreColor', color)) {
      ...
      console.log("No selection, setting text color for entire textbox to:", color);
  ...
  const handleFontSizeChange = (newSize: number) => {
    ...
    const currentStyle = textStyles.find(style => style.id === item.textStyle);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      ...
      const dropdownButton = document.querySelector('.text-style-dropdown-button');
  ...
  return (
    <div
      className={`bg-white rounded-md shadow-md border border-gray-200 overflow-x-auto text-style-menu ${className}`}
    ...
    >
      <div className="flex items-center h-10 px-1.5 gap-0.5">
        {/* Text Style Dropdown */}
        <div className="relative ">
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              console.log("Style dropdown button clicked, current state:", isStyleDropdownOpen);
              setIsStyleDropdownOpen(!isStyleDropdownOpen);
              console.log("Style dropdown state after click:", !isStyleDropdownOpen);
            }}
            className="flex items-center space-x-1 px-2 py-1 rounded hover:bg-gray-100 text-black"
            title="Text Style"
          >
            <span className="text-sm truncate w-20">
              {item.textStyle ? 
                textStyles.find(style => style.id === item.textStyle)?.label || 'Style' 
                : 'Style'}
            </span>
            <svg
        ...
        <button
          onClick={toggleBold}
          className={`p-1.5 rounded hover:bg-gray-100 text-black ${!hasTextSelection ? 'opacity-50 cursor-not-allowed' : ''} ${item.fontWeight === 'bold' ? 'text-gray-700' : ''}`}
          title={hasTextSelection ? "Bold" : "Select text first"}
          disabled={!hasTextSelection}
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M6 12h8a4 4 0 0 0 0-8H6v8zm0 0v8h9a4 4 0 0 0 0-8H6z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>

        <button
          onClick={toggleItalic}
          className={`p-1.5 rounded hover:bg-gray-100 text-black ${!hasTextSelection ? 'opacity-50 cursor-not-allowed' : ''} ${item.fontStyle === 'italic' ? 'bg-gray-100' : ''}`}
        ...
        <div className="flex items-center space-x-1">
          ...
          <div className="w-10 text-center text-black">{item.fontSize || 16}</div>
          <button
        ...
        <div className="relative group">
          <button
            className="p-1.5 rounded hover:bg-gray-100 flex items-center text-black"
            title="Text Color"
          >
            <div className="w-4 h-4 rounded-sm border border-gray-300" style={{ backgroundColor: item.textColor || '#000000' }}></div>
          </button>
          <div className="absolute top-full left-0 mt-1 hidden group-hover:block bg-white shadow-lg rounded-md border border-gray-200 p-2 z-10">
            <div className="grid grid-cols-3 gap-1">
              {['#000000', '#FFFFFF', '#4B5563', '#EF4444', '#F59E0B', '#10B981', '#3B82F6', '#8B5CF6', '#EC4899'].map(color => (
                <button
                  key={color}
                  onClick={() => setTextColor(color)}
                  className={`w-6 h-6 rounded-full border text-black ${item.textColor === color ? 'ring-2 ring-blue-500 ring-offset-1' : ''}`}
        ...
        <button
          ...
          className={`p-1.5 rounded hover:bg-gray-100 text-black ${!hasTextSelection ? 'opacity-50 cursor-not-allowed' : ''} ${item.textDecoration === 'underline' ? 'bg-gray-100' : ''}`}
          title={hasTextSelection ? "Underline" : "Select text first"}
          disabled={!hasTextSelection}
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M6 3v7a6 6 0 0 0 12 0V3M4 21h16" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
        ...
        <button
          ...
          className={`p-1.5 rounded hover:bg-gray-100 text-black ${!item.textAlign || item.textAlign === 'left' ? 'bg-gray-100' : ''}`}
          title="Align Left"
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 6h16M4 12h10M4 18h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>

        <button
          onClick={() => setTextAlign('center')}
          className={`p-1.5 rounded hover:bg-gray-100 text-black ${item.textAlign === 'center' ? 'bg-gray-100' : ''}`}
          title="Align Center"
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 6h16M7 12h10M6 18h12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>

        <button
          onClick={() => setTextAlign('right')}
          className={`p-1.5 rounded hover:bg-gray-100 text-black ${item.textAlign === 'right' ? 'bg-gray-100' : ''}`}
          title="Align Right"
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 6h16M10 12h10M6 18h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>

        <button
          onClick={() => setTextAlign('justify')}
          className={`p-1.5 rounded hover:bg-gray-100 text-black ${item.textAlign === 'justify' ? 'bg-gray-100' : ''}`}
          title="Justify"
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M8 6h12M8 12h12M8 18h12M3 6h.01M3 12h.01M3 18h.01" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
        ...
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M8 6h12M8 12h12M8 18h12M3 6h1v4M3 12h2l-2 4h2M3 18h1.5l1-2v4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
        ...
        <button
          ...
          className={`p-1.5 rounded hover:bg-gray-100 text-black ${!hasTextSelection ? 'opacity-50 cursor-not-allowed' : ''} ${item.textDecoration === 'line-through' ? 'bg-gray-100' : ''}`}
          title={hasTextSelection ? "Strikethrough" : "Select text first"}
          disabled={!hasTextSelection}
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 12h16M17.5 8.5C17.5 6.5 15 6 12 6s-5.5.5-5.5 2.5c0 1.5 2 2 3 2.5m8 0c1 .5 3 1 3 2.5 0 2-2.5 2.5-5.5 2.5s-5.5-.5-5.5-2.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
