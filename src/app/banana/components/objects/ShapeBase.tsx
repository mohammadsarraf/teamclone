...
export const getBaseStyles = (item: GridItem) => {
  ...
  if (
    ...
    const cssStyleValue =
      item.borderStyle === "dash" ? "dashed" : item.borderStyle;
  ...
  return {
    ...
    backgroundColor:
      item.type === "textbox"
        ? "transparent"
        : item.backgroundColor || "#3B82F6",
    color: item.textColor || "#FFFFFF",
    borderRadius:
      item.shapeType === "circle" ? "50%" : `${item.borderRadius || 8}px`,
...
export const GroupIndicator: React.FC = () => (
  <div className="absolute -left-2 -top-2 flex size-5 items-center justify-center rounded-full bg-purple-500 text-xs text-white">
    G
  </div>
);
...
export default function ShapeBase({
  ...
  const shadowClass =
    item.shadow &&
    item.shadow !== "custom" &&
    !item.shadow.startsWith("object-blur") &&
