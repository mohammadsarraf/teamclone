        ...
        if (container) {
          const offsetX =
            (container.clientWidth -
              (Math.floor(container.clientWidth / (gridSize + colGap)) *
                (gridSize + colGap) -
                colGap)) /
            2;
          const offsetY =
            (container.clientHeight -
              (Math.floor(container.clientHeight / (gridSize / 2 + rowGap)) *
                (gridSize / 2 + rowGap) -
                rowGap)) /
            2;
          setPosition((prev) => ({
            x:
              Math.round((prev.x - offsetX) / (gridSize + colGap)) *
                (gridSize + colGap) +
              offsetX,
            y:
              Math.round((prev.y - offsetY) / (gridSize / 2 + rowGap)) *
                (gridSize / 2 + rowGap) +
              offsetY,
          }));
        }
      ...
      {dragging && (
        <div
          style={{
            position: "absolute",
            left:
              Math.round((position.x - offsetX) / (gridSize + colGap)) *
                (gridSize + colGap) +
              offsetX,
            top:
              Math.round((position.y - offsetY) / (gridSize / 2 + rowGap)) *
                (gridSize / 2 + rowGap) +
              offsetY,
            width: size.width,
            height: size.height,
