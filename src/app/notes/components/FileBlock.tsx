...
export const FileBlock: React.FC<FileBlockProps> = ({ type, data, filename, metadata }) => {
  const handleFileOpen = useCallback(() => {
    try {
      ...
      const mimeType = extension === 'pdf' ? 'application/pdf' : `application/${extension}`;
        ...
        <div className="group overflow-hidden rounded-lg border border-zinc-700 bg-zinc-800/50 transition-all hover:border-zinc-600 hover:bg-zinc-800">
          ...
          <div className="flex items-center gap-3 border-t border-zinc-700/50 px-4 py-2">
            ...
            <div className="flex flex-1 items-center gap-2 truncate">
              ...
              {metadata?.size && (
                <span className="shrink-0 text-xs text-zinc-500">({metadata.size} MB)</span>
              )}
            </div>
            {metadata?.uploadDate && (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {metadata?.size && (
              <span className="text-xs text-zinc-400">({metadata.size} MB)</span>
            )}
          </div>
